...
UberMaterial::UberMaterial() : Material(L"./Resources/Effects/UberShader.fx"),
	...
	m_FresnelHardness(0)
{
	m_MaterialTypeName = L"Ubermaterial";

	RegisterVariable<DirectX::XMFLOAT4>(this, &UberMaterial::GetDiffuseColor, &UberMaterial::SetDiffuseColor, "DiffuseColor");
	RegisterVariable<bool>(this, &UberMaterial::GetDiffuseTextureENabled, &UberMaterial::EnableDiffuseTexture, "Enable DiffuseTexture");
	RegisterVariable<TextureStruct*>(this, &UberMaterial::GetDiffuseTextureStruct, &UberMaterial::SetDiffuseTextureStruct, "Diffuse Texture");
...
void UberMaterial::SetDiffuseTexture(const std::wstring& assetFile)
{
	...
	m_DiffuseTexturePath = assetFile;
}

void UberMaterial::SetDiffuseTextureStruct(TextureStruct* texture)
{
	m_pDiffuseTexture = ContentManager::Load<TextureData>(texture->path);
	m_pDiffuseTexStrut = texture;

}

void UberMaterial::SetSpecularLevelTextureStruct(TextureStruct* texture)
{
	m_pSpecularLevelTexture = ContentManager::Load<TextureData>(texture->path);
	m_SpecularTextureStruct = texture;

}

void UberMaterial::SetNormalMapTextureStruct(TextureStruct* texture)
{
	m_pNormalMappingTexture = ContentManager::Load<TextureData>(texture->path);
	m_NormalTextureStruct = texture;
}
...
void UberMaterial::SetSpecularLevelTexture(const std::wstring& assetFile)
{
	...
	m_SpecularTexturePath = assetFile;
...
void UberMaterial::SetEnvironmentCube(const std::wstring& assetFile)
{
	...
	m_EnviromentTexturePath = assetFile;

}

void UberMaterial::SetEnvironmentCubeStruct(TextureStruct * texture)
{
	m_pEnvironmentCube = ContentManager::Load<TextureData>(texture->path);
	m_EnviromentTextureStruct = texture;
}

void UberMaterial::SetOpacityTextureStruct(TextureStruct* texture)
{
	m_pOpacityMap = ContentManager::Load<TextureData>(texture->path);
	m_OpacityTextureStruct = texture;
}
...
void UberMaterial::SetOpacityTexture(const std::wstring& assetFile)
{
	...
	m_OpacityTexturePath = assetFile;
}
...
void UberMaterial::WriteToFile(BinaryWriter* writer)
{
	writer->WriteLongString(GetName());
	writer->Write((unsigned char)5);

	//light
	writer->Write(m_LightDirection);

	//diffuse
	writer->Write(m_bDiffuseTexture);
	if (m_pDiffuseTexStrut)
	{
		writer->WriteString(m_pDiffuseTexStrut->name);
	}
	else
	{
		writer->WriteString("");
	}
	writer->Write(m_ColorDiffuse);

	//specular
	writer->Write(m_ColorSpecular);
	writer->Write(m_bSpecularLevelTexture);
	if (m_SpecularTextureStruct)
	{
		writer->WriteString(m_SpecularTextureStruct->name);
	}
	else
	{
		writer->WriteString("");
	}
	writer->Write(m_Shininess);

	//ambient
	writer->Write(m_ColorAmbient);
	writer->Write(m_AmbientIntensity);

	//normal
	writer->Write(m_bFlipGreenChannel);
	writer->Write(m_bNormalMapping);
	if (m_NormalTextureStruct)
	{
		writer->WriteString(m_NormalTextureStruct->name);
	}
	else
	{
		writer->WriteString("");
	...
	//enviro
	writer->Write(m_bEnvironmentMapping);
	if (m_EnviromentTextureStruct)
	{
		writer->WriteString(m_EnviromentTextureStruct->name);
	}
	else
	{
		writer->WriteString("");
	}
	writer->Write(m_ReflectionStrength);
	writer->Write(m_RefractionStrength);
	writer->Write(m_RefractionIndex);

	//opacity
	writer->Write(m_Opacity);
	writer->Write(m_bOpacityMap);
	if (m_OpacityTextureStruct)
	{
		writer->WriteString(m_OpacityTextureStruct->name);
	}
	else
	{
		writer->WriteString("");
	...
	//specblinn
	writer->Write(m_bSpecularBlinn);
	writer->Write(m_bSpecularPhong);

	//fresnel
	writer->Write(m_bFresnelFaloff);
	writer->Write(m_ColorFresnel);
	writer->Write(m_FresnelPower);
	writer->Write(m_FresnelMultiplier);
	writer->Write(m_FresnelHardness);
