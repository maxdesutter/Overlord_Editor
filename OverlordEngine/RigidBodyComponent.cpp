...
{
	m_ComponentClassName = "RigidBodyComponent";
	RegisterVariable<bool>(this, &RigidBodyComponent::IsKinematic, &RigidBodyComponent::SetKinematic, "Kinematic");
...
void RigidBodyComponent::RemoveCollider(ColliderComponent* collider)
{
	for(size_t i=0;i<m_Colliders.size();i++)
	{
		if(m_Colliders[i] == collider)
		{
			...
			m_pActor->detachShape(*collider->GetShape());
...
void RigidBodyComponent::SetKinematic(bool isKinematic)
{
	...
	{
		if (physx::PxGeometryType::ePLANE == col->GetShape()->getGeometryType())
		{
			m_isKinematic = true;
...
void RigidBodyComponent::WriteToFile(BinaryWriter* writer)
{
