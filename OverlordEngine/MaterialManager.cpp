...
Material* MaterialManager::GetMaterialNoError(UINT materialId, bool forceDeferred) const
{
...
std::vector<UINT> MaterialManager::GetAllMaterialKeys(bool defered)
{
	if(defered)
	{
		std::vector<UINT> retval;
		for (auto const& material : m_pMaterials_Deferred) {
			retval.push_back(material.first);
		}
		return retval;
	}
	std::vector<UINT> retval;
	for (auto const& material : m_pMaterials) {
		retval.push_back(material.first);
	}
	return retval;
}
