...
void GameObject::RemoveComponent(BaseComponent* pComp, bool Delete )
{
...
void GameObject::SaveToFile(BinaryWriter* writer)
{
	//write name
	writer->WriteLongString(m_Name);
	//transform
	writer->Write(m_pTransform->GetPosition());
	DirectX::XMVECTOR rotVect = DirectX::XMLoadFloat4(&m_pTransform->GetRotation());

	writer->Write(rotVect);	
	writer->Write(m_pTransform->GetScale());

	//components
		//num components
	writer->Write((unsigned char)m_pComponents.size());

	for(auto comp:m_pComponents)
	{
		comp->WriteToFile(writer);
	}

	//children
	writer->Write((unsigned char)m_pChildren.size());
	for (auto child : m_pChildren)
	{
		child->SaveToFile(writer);
	}	
}