...
{
	m_ComponentClassName = "TerrainComponent";
}
...
void TerrainComponent::WriteToFile(BinaryWriter* writer)
{
	writer->Write((unsigned char)7);
	writer->WriteLongString(m_HeightMapFile);
	writer->WriteLongString(m_TextureFile);

	writer->Write(m_NrOfRows);
	writer->Write(m_NrOfColumns);
