...
std::wstring BinaryReader::GetFinalWord(UINT length)
{
	if (m_pReader)
	{
		m_pReader->seekg(0, std::ios::end);
		int size = sizeof(wchar_t)* length;
		MoveBufferPosition(-size -sizeof(UINT));
...
std::string BinaryReader::ReadRegString()
{
	if (m_pReader == nullptr)
		Logger::LogError(L"BinaryReader doesn't exist!\nUnable to read binary data...");
