...
{
	SetAnimation(0);
	RegisterVariable<bool>(this, &ModelAnimator::GetPlaying, &ModelAnimator::SetPlaying, "Playing");
	RegisterVariable<UINT>(this, &ModelAnimator::GetAnimationClipRef, &ModelAnimator::SetAnimationClipRef, "Clip");
	RegisterVariable<float>(this, &ModelAnimator::GetAnimationSpeed, &ModelAnimator::SetAnimationSpeed, "speed");
...
void ModelAnimator::SetAnimationClipRef(UINT clip)
{
	if (clip >= m_pMeshFilter->m_AnimationClips.size())
	{
		return;
	}
	m_ClipSet = false;
	m_AnimationClipNumber = clip;


	SetAnimation(m_pMeshFilter->m_AnimationClips[clip]);
}
